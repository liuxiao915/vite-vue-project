  ...
  <div class="left-panel">
    <el-collapse v-model="activeNames">
      <el-collapse-item name="1" title="布局组件">
        <draggable tag="ul" :list="containersFields" item-key="key" :group="{name: 'dragGroup', pull: 'clone', put: false}" :move="checkFieldMove" :clone="handleClone" :sort="false">
      ...
      <el-collapse-item name="2" title="基础组件">
        <draggable tag="ul" :list="basicFields" item-key="key" :group="{name: 'dragGroup', pull: 'clone', put: false}" :move="checkFieldMove" :clone="handleClone" :sort="false">
          <template #item="{ element }">
            <li>{{ element.label }}</li>
          </template>
        </draggable>
      ...
      <el-collapse-item name="3" title="高级组件">
        <draggable tag="ul" :list="advancedFields" item-key="key" :group="{name: 'dragGroup', pull: 'clone', put: false}" :move="checkFieldMove" :clone="handleClone" :sort="false">
          <template #item="{ element }">
            <li>{{ element.label }}</li>
          </template>
        </draggable>
      ...
      <el-collapse-item name="4" title="自定义扩展组件">
        <draggable tag="ul" :list="customFields" item-key="key" :group="{name: 'dragGroup', pull: 'clone', put: false}" :move="checkFieldMove" :clone="handleClone" :sort="false">
          <template #item="{ element }">
            <li>{{ element.label }}</li>
          </template>
        </draggable>
