...
import { deepClone, generateId } from "@/utils/index"
import { containers, advancedFields, basicFields, customFields } from "./widget-panel/widgetsConfig.js"
...
export function createDesigner(vueInstance) {
  let defaultFormConfig = deepClone({
    modelName: 'formData',
    refName: 'vForm',
    rulesName: 'rules',
    labelWidth: 80,
  ...
  return {
    ...
    mergeTableWholeRow(rowArray, colArray, rowIndex, colIndex) { //需要考虑操作的行存在已合并的单元格！！
      ...
      if (unmatchedFlag) {
        this.vueInstance.$message.info(this.vueInstance('存在行高不一致的单元格, 无法合并整行.'))
        return
      }
