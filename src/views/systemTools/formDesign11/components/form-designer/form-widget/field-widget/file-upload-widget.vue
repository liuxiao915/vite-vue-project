<template>
  <form-item-wrapper :designer="designer" :field="field" :rules="rules" :design-state="designState" :parent-widget="parentWidget" :parent-list="parentList" :index-of-parent-list="indexOfParentList" :sub-form-row-index="subFormRowIndex" :sub-form-col-index="subFormColIndex" :sub-form-row-id="subFormRowId">
    <!-- el-upload增加:name="field.options.name"后，会导致又拍云上传失败！故删除之！！ -->
    <el-upload ref="fieldEditor" :disabled="field.options.disabled" :style="styleVariables" class="dynamicPseudoAfter" :action="realUploadURL" :headers="uploadHeaders" :data="uploadData" :with-credentials="field.options.withCredentials" :multiple="field.options.multipleSelect" :file-list="fileList" :show-file-list="field.options.showFileList" :class="{'hideUploadDiv': uploadBtnHidden}" :limit="field.options.limit" :on-exceed="handleFileExceed" :before-upload="beforeFileUpload" :on-success="handleFileUpload" :on-error="handleUploadError">
      <template #tip>
        <div class="el-upload__tip" v-if="!!field.options.uploadTip">{{field.options.uploadTip}}</div>
      </template>
      <template #default>
        <svg-icon svg-name="el-plus" /><i class="el-icon-plus avatar-uploader-icon"></i>
      </template>
      <template #file="{ file }">
        <div class="upload-file-list">
          ...
          <a :href="file.url" download="" target="_blank">
            <span class="el-icon-download file-action" :title="i18nt('render.hint.downloadFile')">
              <svg-icon svg-name="el-download" />
            </span></a>
          <span class="file-action" :title="i18nt('render.hint.removeFile')" v-if="!field.options.disabled" @click="removeUploadFile(file.name, file.url, file.uid)"><svg-icon svg-name="el-delete" /></span>
        </div>
      </template>
    </el-upload>
  </form-item-wrapper>
  ...
  computed: {
    realUploadURL() {
      ...
      if (!!uploadURL && ((uploadURL.indexOf('DSV.') > -1) || (uploadURL.indexOf('DSV[') > -1))) {
        return evalFn(this.field.options.uploadURL, null)
      }
...
@import '../../../../styles/global.less'; /* form-item-wrapper已引入，还需要重复引入吗？ */
