<template>
  <form-item-wrapper :designer="designer" :field="field" :rules="rules" :design-state="designState" :parent-widget="parentWidget" :parent-list="parentList" :index-of-parent-list="indexOfParentList" :sub-form-row-index="subFormRowIndex" :sub-form-col-index="subFormColIndex" :sub-form-row-id="subFormRowId">
    ...
    <div>
      <!-- <quill-editor v-model:value="fieldModel" :options="editorOption" :disabled="field.options.disabled" @blur="handleRichEditorBlurEvent" @focus="handleRichEditorFocusEvent" @change="handleRichEditorChangeEvent" :style="!!field.options.contentHeight ? `height: ${field.options.contentHeight};`: ''"></quill-editor> -->
    </div>
...
import { deepClone } from "@/views/systemTools/formDesign11/utils/util";
