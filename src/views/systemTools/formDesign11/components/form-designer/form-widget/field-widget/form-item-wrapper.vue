...
<template>
  <div class="field-wrapper" :class="{'design-time-bottom-margin': !!this.designer}">
    <el-form-item v-if="!!field.formItemFlag && (!field.options.hidden || (designState === true))" :label="label" :label-width="labelWidth + 'px'" :title="field.options.labelTooltip" :rules="rules" :prop="getPropName()" :class="[selected ? 'selected' : '', labelAlign, customClass, field.options.required ? 'required' : '']" @click.stop="selectField(field)">
          ...
          <template v-if="field.options.labelIconPosition === 'front'">
            <template v-if="!!field.options.labelTooltip">
              <el-tooltip :content="field.options.labelTooltip" effect="light">
                <svg-icon :svg-name="field.options.labelIconClass" /></el-tooltip>{{label}}</template>
            <template v-else>
              <svg-icon :svg-name="field.options.labelIconClass" />{{label}}</template>
          </template>
          <template v-else-if="field.options.labelIconPosition === 'rear'">
            <template v-if="!!field.options.labelTooltip">
              {{label}}<el-tooltip :content="field.options.labelTooltip" effect="light">
                <svg-icon :svg-name="field.options.labelIconClass" /></el-tooltip></template>
            <template v-else>
              {{label}}<svg-icon :svg-name="field.options.labelIconClass" /></template>
          </template>
        </span>
      </template>
      <slot></slot>
    ...
    <template v-if="!!this.designer">
      <div class="field-action" v-if="designer.selectedId === field.id">
        <i :title="i18nt('designer.hint.selectParentWidget')" @click.stop="selectParentWidget(field)"><svg-icon svg-name="el-back" /></i>
        <i v-if="!!parentList && (parentList.length > 1)" :title="i18nt('designer.hint.moveUpWidget')" @click.stop="moveUpWidget(field)"><svg-icon svg-name="el-move-up" /></i>
        <i v-if="!!parentList && (parentList.length > 1)" :title="i18nt('designer.hint.moveDownWidget')" @click.stop="moveDownWidget(field)"><svg-icon svg-name="el-move-down" /></i>
      ...
      <div class="drag-handler background-opacity" v-if="designer.selectedId === field.id">
        ...
        <i v-if="field.options.hidden === true"><svg-icon svg-name="el-hide" /></i>
      </div>
    </template>
  </div>
</template>
