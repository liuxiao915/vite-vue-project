<template>
  <el-select v-model="valueTitle" @clear="clearHandle" ref="treeSelect" filterable :filter-method="filterSelect" @visible-change="clearSelect" placeholder="请选择">
    <!-- <el-input class="selectInput" placeholder="请选择" v-model="filterText"></el-input> -->
    <el-option :value="valueTitle" :label="valueTitle">
      <el-tree id="tree-option" ref="selectTree" :data="treeOptions" :props="props" :node-key="props.value" :default-expanded-keys="defaultExpandedKey" :filter-node-method="filterNode" @node-click="handleNodeClick">
      </el-tree>
    </el-option>
  </el-select>
</template>
...
export default {
  ...
  methods: {
    ...
    initHandle() {
      if (this.valueId) {
        let info = this.findFirstDeptFarm2(this.treeOptions);
    ...
    filterSelect(val) {
      if (val) {
        this.treeOptions = this.filterTreeByName(this.treeOptions, val);
      } else {
        this.treeOptions = this.treeOptions;
      }
    },
